...
df = read.csv('test2.csv',stringsAsFactors = FALSE)
df$input = factor(df$input,ordered=TRUE)
df$output = factor(df$output,levels=c('parc-stats','tractmeasures','network','psd'))
df$timeseries_extraction = factor(df$timeseries_extraction,levels=c('timeseries','epochs','z'))

df$parcellation = factor(df$parcellation,levels=c('freesurfer','z'),ordered=TRUE)
